 ...
 * 注意需要添加mapperScan注解
 */
@Slf4j public class UserTest extends MybatisPlusBootstrapTest {
     ...
     * 查询单个用户
     */
    @Test public void testQueryUser() {
        log.info("根据ID查询------------------------------------------------------------------------");
    ...
    @Test public void testQueryList() {
        ...
        List<User> inSql = userMapper
                .selectList(new LambdaQueryWrapper<User>().inSql(User::getId, "select user_id from address"));
    ...
    @Test public void testUpdate() {
        ...
        assertThat(userMapper
                .update(new User(), Wrappers.<User>lambdaUpdate().set(User::getName, "Jone").eq(User::getId, 1)))
                .isGreaterThan(0);
    ...
    public void testOrderBy() {
        List<Map<String, Object>> maps = 
                userMapper.selectMaps(Wrappers.<User>lambdaQuery().orderByAsc(User::getId));
